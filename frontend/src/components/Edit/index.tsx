...
import { getMockImageById } from '../../mock/images';
...
const ImageEditor = observer(() => {
  ...
  const applyFilterStyles = useCallback(() => {
    if (!cropperRef.current?.cropper) return;
    const imgElement = cropperRef.current?.cropper?.image;
    if (imgElement) {
